...
import { Clock9, Clock3, Clock12 } from 'lucide-react';
...
export function BusinessHoursInput({ value = [], onChange, className }: BusinessHoursInputProps) {
  const [businessHours, setBusinessHours] = useState<BusinessHours[]>(
    value.length > 0 ? value : DAYS.map(day => ({
      ...DEFAULT_HOURS,
  ...
  const getDayHours = (dayId: DayOfWeek) => {
    return businessHours.find(h => h.dayOfWeek === dayId) || DEFAULT_HOURS;
  };
    ...
    <div className={cn('space-y-4', className)}>
      <div className="space-y-2">
        ...
        <p className="text-sm text-muted-foreground">
          Defina os horários em que sua loja está aberta.
        </p>
      </div>
